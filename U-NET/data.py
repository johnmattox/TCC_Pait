...
def getData(path, foldlist):
    ...
    for i, image_path in enumerate(foldlist):
        ...
        mask = imread(os.path.join('', file_name + '_mask' + '.jpg'), as_grey=True)
        masks.append(mask)

        image = imread(os.path.join('', file_name + '_8' + '.jpg'), as_grey=True)
        images.append(image)

        #image16 = imread(os.path.join('', file_name + '.jpg'), as_grey=True)
