...
def train_and_predict(bit):
    print('-' * 30)
    print('Loading and preprocessing train data (bit = ' + str(bit) + ') ...')
    print('-' * 30)
    ...
    print('-' * 30)
    if os.path.exists('weights_unet_' + str(bit) + '.h5'):
        print("Hum, it seems we have some trained weights here already! Let's start the training based on them...")
        model.load_weights('weights_unet_' + str(bit) + '.h5')
            
    model.fit(imgs_bit_train, imgs_bit_mask_train, batch_size=batchsize, epochs=epochs, verbose=1, shuffle=True,
    ...
    if bit == 8:
        ...
        if not os.path.exists(pred_dir):
            os.mkdir(pred_dir)
        for image, original, image_id in zip(imgs_mask_test, imgs_bit_test, imgs_bit_id_test):
